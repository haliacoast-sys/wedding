/**
 * useTasksRealtime.ts — tasks 테이블 구독.
 *
 ...
 *    이것이 없으면 내 변경의 메아리가 돌아와 내 낙관적 상태를 이전 값으로 되돌린다.
 *
...
import { tasksKey } from './tasksApi'
import { hasLocalWrites, onLocalWritesSettled } from './writeGuard'
import type { Task } from './types'

export type RealtimeState = 'connecting' | 'live' | 'offline'

/** 채널 이름은 마운트마다 달라야 한다. StrictMode 에서 이전 채널이 아직 정리되기 전에 새 채널이 생긴다. */
let channelSeq = 0

const applyChange = (qc: QueryClient, payload: RealtimePostgresChangesPayload<Task>): void => {
  qc.setQueryData<Task[]>(tasksKey, (previous) => {
    // 아직 첫 조회조차 끝나지 않았다면 캐시를 만들지 않는다.
    // 여기서 만들면 그 행 하나만 있는 목록이 완성본인 척하게 된다.
    if (!previous) return previous

    if (payload.eventType === 'DELETE') {
      const removedId = (payload.old as Partial<Task> | null)?.id
      return removedId ? previous.filter((t) => t.id !== removedId) : previous
    }

    const row = payload.new as Task | null
    if (!row?.id) return previous
    const index = previous.findIndex((t) => t.id === row.id)
...
export const useTasksRealtime = (): RealtimeState => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`checklist-tasks-${channelSeq}`)
      .on<Task>(
        'postgres_changes',
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          if (subscribedOnceRef.current) {
            ...
            else void qc.invalidateQueries({ queryKey: tasksKey })
          }
