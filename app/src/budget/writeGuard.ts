/**
 * writeGuard.ts — "지금 내가 쓰는 중인가?" 를 세는 카운터.
 *
 * 왜 필요한가.
 *   낙관적 업데이트는 캐시를 먼저 고쳐 놓고 서버 응답을 기다린다. 그 사이에
 *   Realtime 이벤트가 도착해 캐시를 서버 상태로 덮어쓰면, 방금 고친 금액이
 *   눈앞에서 이전 값으로 되돌아간다. 상대가 만든 변경이든 내 변경이 돌아온
 *   메아리든 결과는 같다.
 *
 *   그래서 쓰기가 진행 중인 동안에는 Realtime 이 캐시를 건드리지 않게 막고,
 *   "밀린 변경이 있었다"는 사실만 기록해 뒀다가 쓰기가 전부 끝난 뒤 한 번만 맞춘다.
 *
 * useIsMutating() 을 쓰지 않는 이유:
 *   그 값은 리렌더를 거쳐야 갱신되므로 onMutate 와 실제 반영 사이에 틈이 생긴다.
 *   여기서는 onMutate 첫 줄에서 동기적으로 올리고 onSettled 에서 내리므로 틈이 없다.
 *   React 상태가 아니라 조율용 카운터라 모듈 스코프가 맞다.
 *
 * 체크리스트 모듈에도 같은 이름의 파일이 있지만 일부러 공유하지 않는다.
 * 카운터를 공유하면 체크리스트 쓰기가 예산 화면의 Realtime 반영까지 막는다.
 */
